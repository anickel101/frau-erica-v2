...
// Reads a JPEG's real dimensions out of its SOF marker. Every Keepers
// header photo is already on the CDN, so nothing needs uploading -- but
// their Images rows have no width/height, and 152 of the archive's 566
// rows are in the same state. Emitting real dimensions lets the renderer
// reserve space, which is what removes layout shift as photos load.
...
// A header photo lives in one of the two export folders. Which one is an
// accident of how the archive was exported, not something meaningful.
function findImage(file: string): string | null {
  for (const dir of [KEEPERS_DIR, IMAGES_DIR]) {
...
// Idempotent by design: INSERT ... WHERE NOT EXISTS for photos the
// archive doesn't have yet, and UPDATE for the handful it already has
// but without dimensions. Re-running changes nothing the second time,
// and no image_id is ever hardcoded -- AUTOINCREMENT assigns them, and
// the Recipes rows resolve header_image_id by filename.
function imagesSql(): string[] {
  const lines: string[] = [
    '-- Header photographs. All 36 are already on the CloudFront bucket;',
    '-- these are the Images rows, with real pixel dimensions read from',
    '-- the files. Captions are deliberately left NULL (the Archivist:',
    '-- "most pages work better without them").',
  ]
  for (const file of [...headers.keys()].sort()) {
    const onDisk = findImage(file)
    const size = onDisk ? jpegSize(onDisk) : null
    const width = size ? String(size.width) : 'NULL'
    const height = size ? String(size.height) : 'NULL'
    lines.push(
      `INSERT INTO Images (url, width, height, is_published) ` +
        `SELECT ${quote(file)}, ${width}, ${height}, 1 ` +
        `WHERE NOT EXISTS (SELECT 1 FROM Images WHERE url = ${quote(file)});`,
    )
    if (size) {
      lines.push(
        `UPDATE Images SET width = ${width}, height = ${height} ` +
          `WHERE url = ${quote(file)} AND (width IS NULL OR height IS NULL);`,
      )
    }
  }
  lines.push('')
